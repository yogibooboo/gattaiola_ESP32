...
void u8g2_prova() {
  char s[2] = " ";
  uint8_t x, y;
  char buffer[10]; // Come sopra

itoa(frequenza, buffer, 10);

// Ora `buffer` contiene la rappresentazione testuale di `valore`
//const char* stringa = buffer;
  u8g2.drawStr( 0, 0, buffer);
  if (statoconta==1) u8g2.drawStr( 0, 40, "COUNT");
  else u8g2.drawStr( 0, 40, "STOP");
  /*for( y = 0; y < 6; y++ ) {
    for( x = 0; x < 16; x++ ) {
      s[0] = y*16 + x + 32;
      u8g2.drawStr(x*7, y*10+10, s);
